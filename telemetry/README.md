...
This folder contains ready-to-use telemetry assets for `events-presenter`:

- `dashboards/events-presenter-overview.dashboard.json`: Grafana dashboard with example panels
...
1. `events-presenter` running with OpenTelemetry enabled:
...
2. OpenTelemetry Collector reachable from `events-presenter`
...
3. Upload `dashboards/events-presenter-overview.dashboard.json`
...
- DB connect latency (p50/p95)
- `/events` request rate and error rate
- `/events` latency (p50/p95)
- Resources returned throughput
- Listener notifications/jobs throughput
- Listener DB fetch latency and failures
- SSE clients active and connect/disconnect trends
- SSE broadcast vs dropped event rates
...
4. Point `events-presenter` to the Collector service:
